 ...
 *
 * Swift source: Sources/KanbanCodeCore/UseCases/RemoteShellManager.swift (366 lines)
 */
...
export function deploy(): void {
  ...
  fs.writeFileSync(script, REMOTE_SHELL_SCRIPT, 'utf-8');
 ...
 */
export function shellOverridePath(): string {
  return path.join(remoteDir(), 'zsh');
}
 ...
 */
export function remoteDirPath(): string {
  return remoteDir();
}
 ...
 */
export function setupEnvironment(_remote: RemoteSettings, _projectPath: string): Record<string, string> {
  return {}; // Script reads config from ~/.kanban-code/settings.json directly
}

// MARK: - Paths

function remoteDir(): string {
  return path.join(os.homedir(), '.kanban-code', 'remote');
}

function scriptPath(): string {
  return path.join(remoteDir(), 'remote-shell.sh');
}
...
const REMOTE_SHELL_SCRIPT = `#!/bin/bash
#
